...
/**
 * Generates a branded 1200x630 Open Graph image per docs/articles page at
 * build time (postBuild) and rewrites that page's og:image / twitter:image
  ...
  return {
    ...
    async postBuild({outDir, routesPaths, siteConfig}) {
      ...
      const targets = (routesPaths || []).filter(
        (r) =>
          (r.startsWith('/docs/') || r.startsWith('/articles/') || r.startsWith('/cheatsheets/')) &&
